...
function getMedia($targetdir){
    ...
    if(is_file($targetdir . '/titles.json')){
        $titles = file_get_contents($targetdir . '/titles.json');
...
function getSlides($targetdir,$relpath){
    ...
    foreach($media as $filename => $title){
            $thumbname = preg_replace('/(.*)\.([a-z]{3})/',"$1" . "_thumb." . "$2",$basename);
            if(!is_file("$relpath/$thumbname")){
            ...
            $html .= "<span class='thumbnailinner'>\n";
                ...
                $html .= "</a>";
            $html .= "</span>\n";
            $html .= "<span class='filename'>".$filename."</span></div>\n";
    }
    if(count($media) === 0){
...
function printThumbnail($targetdir){
    ...
    $thumb = preg_replace('/(.*)\.([a-z]{3})$/',"$1" . "_thumb." . "$2",$basename);
